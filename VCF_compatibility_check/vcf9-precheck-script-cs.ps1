...
if (-not (Get-Module -ListAvailable -Name VMware.PowerCLI)) {
    ...
    try {
        ...
        Write-Host "인터넷이 되는 PC에서 아래 명령어를 실행하여 파일 다운로드 후 이 서버로 복사하세요:" -ForegroundColor Yellow
        Write-Host "  Save-Module -Name VMware.PowerCLI -Path C:\Temp_Modules" -ForegroundColor Yellow
        Write-Host "  복사 경로: C:\Program Files\WindowsPowerShell\Modules" -ForegroundColor Yellow
        Write-Host "===============================================================================" -ForegroundColor Red
        Exit
    }
} else {
...
# ----------------------------------------------------
# --- 호스트별 라이선스 키 조회 준비: LicenseAssignmentManager는 한 번만 가져오고, 실제 조회는 호스트별로 수행 ---
# (참고: QueryAssignedLicenses에 entityId를 비워서 "전체 일괄 조회"하는 방식은 PowerCLI의 Get-View 바인딩에서
#  결과가 비어 나오는 경우가 보고되어 있어, 호스트별 개별 조회 방식을 사용합니다 - vSphere 8 포함 전 버전에서 안정적으로 동작)
$LicAssignMgr = $null
try {
    ...
    Write-Host "[WARN] 라이선스 관리자(LicenseAssignmentManager)를 가져오지 못했습니다: $($_.Exception.Message)" -ForegroundColor Yellow
}
...
foreach ($HostObj in $VMHosts) {
    ...
    if ($HostObj.ConnectionState -eq "Connected") {
        ...
        $HWReport += [PSCustomObject]@{
            ...
            "License_Key"        = if ($LicAssignMgr) {
                                        $LicAssignResult = $null
                                        try { $LicAssignResult = $LicAssignMgr.QueryAssignedLicenses($HostObj.ExtensionData.MoRef.Value) } catch { $LicAssignResult = $null }
                                        if ($LicAssignResult -and @($LicAssignResult).Count -gt 0) { $LicAssignResult[0].AssignedLicense.LicenseKey } else { "N/A" }
                                    } else { "N/A" }
...
foreach ($H in $ConnectedHosts) {
    ...
    $NicList = if ($esxcli) { try { $esxcli.network.nic.list.Invoke() } catch { $null } } else { $null }

    foreach ($P in $H.ExtensionData.Config.Network.Pnic) {
        $MTU = "N/A"; $Driver = "N/A"; $AutoNeg = "N/A"; $Model = "N/A"
        ...
        if ($NicList) {
            $nicCli = $NicList | Where-Object { $_.Name -eq $P.Device }
            if ($nicCli) {
                $MTU = $nicCli.MTU
        ...
        if ($esxcli) {
            try {
                ...
                $nicArgs.nicname = $P.Device
                $nicDetail = $esxcli.network.nic.get.Invoke($nicArgs)
                
                if ($nicDetail -and $nicDetail.DriverInfo) { 
                    if ($nicDetail.DriverInfo.Version) { $DriverVersion = $nicDetail.DriverInfo.Version }
                    if ($nicDetail.DriverInfo.FirmwareVersion) { $FirmwareVersion = $nicDetail.DriverInfo.FirmwareVersion }
                }
                if ($nicDetail -and $nicDetail.LinkSettings) { 
                    $AutoNeg = $nicDetail.LinkSettings.AutoNegotiate 
                }
            } catch {}
        }
...
function Find-BestHCLMatch {
    param([array]$Table, [string]$Detected, [string[]]$Fields)
...
function Get-VersionedMatch {
    param([array]$Table90, [array]$Table91, [string]$Detected, [string[]]$Fields, [int]$Threshold)
    $Match90 = Find-BestHCLMatch -Table $Table90 -Detected $Detected -Fields $Fields
    $Match91 = Find-BestHCLMatch -Table $Table91 -Detected $Detected -Fields $Fields
...
if (-not $HasAnyHCLData) {
    ...
    # --- 1) Server 모델 (제조사+모델명 유사도) & 2) CPU (모델 시리즈/세대) 호환성 : ESXi 9.0 / 9.1 각각 판정 ---
    foreach ($HW in $HWReport) {
        $DetectedServerText = "$($HW.Vendor) $($HW.Model)"
        $ServerMatch = Get-VersionedMatch -Table90 $SystemHCL90 -Table91 $SystemHCL91 -Detected $DetectedServerText -Fields @('Partner Name', 'Model') -Threshold $MatchThreshold
    ...
    foreach ($Nic in $PnicReport) {
        ...
        $NicMatch = Get-VersionedMatch -Table90 $IOHCL_Network_90 -Table91 $IOHCL_Network_91 -Detected $Nic.Model -Fields @('Brand Name', 'Model') -Threshold $MatchThreshold
    ...
    foreach ($Ctrl in (@($HbaReport) + @($RaidReport))) {
        ...
        $CtrlMatch = Get-VersionedMatch -Table90 $IOHCL_Storage_90 -Table91 $IOHCL_Storage_91 -Detected $Ctrl.Model -Fields @('Brand Name', 'Model') -Threshold $MatchThreshold
        $VsanMatch = Get-VersionedMatch -Table90 $VsanHCL90 -Table91 $VsanHCL91 -Detected $Ctrl.Model -Fields @('Brand Name', 'Model') -Threshold $MatchThreshold
